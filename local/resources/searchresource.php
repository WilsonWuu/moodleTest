...
$PAGE->set_url(new moodle_url($CFG->RESOURCES_BASEURL.'searchresource.php?' . $_SERVER['QUERY_STRING']));
...
if (!empty($delete)) {
	...
	if (!$userid == $USER->id) {
		require_capability('local/resources:managefileresources', $modcontext);
	}

    $return = new Moodle_url($CFG->RESOURCES_BASEURL.'searchresource.php');
    ...
    if (!$confirm or !confirm_sesskey()) {
        ...
        $formcontinue = new single_button(new moodle_url("$CFG->wwwroot/local/resources/searchresource.php", $optionsyes), get_string('yes'));
...
$PAGE->requires->js(new moodle_url($CFG->RESOURCES_BASEURL.'module.js'));

$PAGE->set_pagelayout('standard');

$PAGE->navbar->add(get_string('resources','local_resources'));
$PAGE->set_title(get_string('resources','local_resources'));
$PAGE->set_heading(get_string('resources','local_resources'));
...
echo $OUTPUT->header();
 ...
 echo $OUTPUT->heading(get_string('resourcelist','local_resources'));
...
/// Print Section or custom info
    ...
    $mods = $modinfo->get_cms();
        ...
        if (($section && (!empty($modinfo->sections[1]) or !empty($section->summary))) or $editing) {
			...
			echo resource_cm_control($SITE, $section->section);

			//echo $renderer->search_resource_search_bar($_GET);

			$rs_list = get_resource_list($SITE, $section, $category);
			$rs_count = count($rs_list);

			$url = 'searchresource.php?' . $_SERVER['QUERY_STRING'];
