...
class core_resources_renderer {
	...
	public function search_resource_search_bar($default_data = null){
		...
		$output .= '<option value="0">'.get_string('allcategory', 'local_resources').'</option>';
    ...
    public function view_resource_list($rs_list = null, $totalcount=0, $page=0, $perpage=10, $url="", $detail = false){
		...
		foreach ($rs_list as $data) {
			...
			if ($data->modname == 'resource') {
				$detailhtml = '<a href="' . $CFG->wwwroot . $CFG->RESOURCES_BASEURL.'view.php?id=' . $data->id . '">' . $data->name . '</a>';
			} else {
				$detailhtml = '<a href="' . $CFG->wwwroot . '/mod/folder/view.php?id=' . $data->id . '">' . $data->name . '</a>';
			}
			...
			if (has_capability('local/resources:managefileresources', context_system::instance())) {
				$buttonedithtml = '<a href="' . $CFG->wwwroot . $CFG->RESOURCES_BASEURL.'editresource.php?update=' . $data->id . '">' . get_string('buttonedit', 'local_resources') . '</a>';
				$buttonedithtml .= '&nbsp&nbsp&nbsp<a href="' . $CFG->wwwroot . $CFG->RESOURCES_BASEURL.'searchresource.php?delete=' . $data->id . '&sesskey='.sesskey().'">' . get_string('buttondelete', 'local_resources') . '</a>';
			}
