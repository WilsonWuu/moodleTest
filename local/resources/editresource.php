...
$url = new moodle_url($CFG->RESOURCES_BASEURL.'editresource.php');
...
if (!empty($add)) {
    ...
    navigation_node::override_active_url(new moodle_url($CFG->RESOURCES_BASEURL.'editresource.php', array('update'=>$update, 'return'=>1)));
    ...
    list($cm, $context, $module, $data, $cw) = videos_can_update_moduleinfo($cm, 'resource');
...
$modmoodleform = "$CFG->dirroot/local/resources/mod_".$module->name."_mod_form_innoverz.php";
...
$mform = new $mformclassname($data, $cw->section, $cm, $course, 'editresource.php', $btnset, get_resource_types(), -1);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    if ($return && !empty($cm->id)) {
        ...
        redirect(new Moodle_url($CFG->RESOURCES_BASEURL.'searchresource.php'));
    }
