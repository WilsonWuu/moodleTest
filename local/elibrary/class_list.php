...
$PAGE->set_url(new moodle_url($CFG->LIBRARY_BASEURL.'class_list.php?' . $_SERVER['QUERY_STRING']));
$PAGE->set_pagelayout('standard');
$PAGE->navbar->add(get_string('library','local_elibrary'), new moodle_url($CFG->LIBRARY_BASEURL.''));
$PAGE->navbar->add(get_string('list_class','local_elibrary'));
...
if(isset($_GET['action'])){
	switch($_GET['action']){
		case 'delete_class':
			...
			$result = delete_class($classid);
			if($result['status'] == 'success'){
				redirect($CFG->wwwroot.$CFG->LIBRARY_BASEURL.'class_list.php?' . base64_decode($_GET['query_string']));
				exit;
			}else{
				echo $OUTPUT->header();
...
echo $OUTPUT->header();
//echo $OUTPUT->heading(get_string('list_class','local_elibrary'));
echo $renderer->start_layout();

$class_list = get_class_list($_GET);
$class_count = count($class_list);
$perpage = 10;
$url = 'class_list.php?' . $_SERVER['QUERY_STRING'];

echo $renderer->view_class_list($class_list, $class_count, $page, $perpage, $url);
