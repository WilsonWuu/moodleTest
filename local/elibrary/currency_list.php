...
$PAGE->set_url(new moodle_url($CFG->LIBRARY_BASEURL.'currency_list.php?' . $_SERVER['QUERY_STRING']));
$PAGE->set_pagelayout('standard');
$PAGE->navbar->add(get_string('library','local_elibrary'), new moodle_url($CFG->LIBRARY_BASEURL.''));
$PAGE->navbar->add(get_string('list_currency','local_elibrary'));
...
if(isset($_GET['action'])){
	switch($_GET['action']){
		case 'delete_currency':
			...
			$result = delete_currency($currencyid);
			if($result['status'] == 'success'){
				redirect($CFG->wwwroot.$CFG->LIBRARY_BASEURL.'currency_list.php?' . base64_decode($_GET['query_string']));
				exit;
			}else{
				echo $OUTPUT->header();
...
echo $OUTPUT->header();
//echo $OUTPUT->heading(get_string('list_currency','local_elibrary'));
echo $renderer->start_layout();

$currency_list = get_currency_list($_GET);
$currency_count = count($currency_list);
$perpage = 10;
$url = 'currency_list.php?' . $_SERVER['QUERY_STRING'];

echo $renderer->view_currency_list($currency_list, $currency_count, $page, $perpage, $url);
