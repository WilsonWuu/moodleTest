...
$PAGE->set_url(new moodle_url($CFG->LIBRARY_BASEURL.'class_new.php?' . $_SERVER['QUERY_STRING']));
$PAGE->set_pagelayout('standard');
$PAGE->navbar->add(get_string('library','local_elibrary'));
$PAGE->navbar->add(get_string('new_class','local_elibrary'));
...
$mform = new class_newedit_form(null, null);
$data = $mform->get_data();
if($data){
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('new_class','local_elibrary'));
$mform->display();
echo $OUTPUT->footer();