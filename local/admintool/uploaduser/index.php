...
$insertrows = optional_param('insertrows', 0, PARAM_INT);
...
require_login();
admin_externalpage_setup('tooluploaduserinnoverz');
require_capability('moodle/site:uploadusers', context_system::instance());

$PAGE->requires->jquery();
$PAGE->requires->js(new moodle_url('/local/admintool/uploaduser/module.js'));
...
$returnurl = new moodle_url('/local/admintool/uploaduser/index.php');
...
if ($creloadcsv && isset($_SESSION['iid'])) {
    //added by Felix
    $filecolumns = $_SESSION['filecolumns'];
    $iid = $_SESSION['iid'];
} */
if (empty($iid)) {

    $mform1 = new admin_uploaduser_form1_innoverz(null, array('insertrows' => 100000));
    ...
    if ($formdata = $mform1->get_data()) {
        ...
        /* //added by Felix
        $_SESSION['filecolumns'] = $filecolumns;
        $_SESSION['iid'] = $iid; */
        // continue to form2
        ...
        echo $OUTPUT->heading_with_help(get_string('uploaduser', 'local_admintool'), 'uploadusers', 'tool_uploaduser');
...
/**
 * fake the moodle system that there are username, firstname and lastname values,
 * as these 3 fields are required in moodle system by default and cannot be set as not default.
 * But actually we do not need them
 * 
 */
$unsetusername = false;
$unsetfirstname = false;
$unsetlastname = false;
if (!isset($filecolumns['username'])) {
    $filecolumns['username'] = 'username';
    $unsetusername = true;
}
if (!isset($filecolumns['firstname'])) {
    $filecolumns['firstname'] = 'firstname';
    $unsetfirstname = true;
}
if (!isset($filecolumns['lastname'])) {
    $filecolumns['lastname'] = 'lastname';
    $unsetlastname = true;
}

$mform2 = new admin_uploaduser_form2(null, array('columns' => $filecolumns, 'data' => array('iid' => $iid, 'previewrows' => $insertrows)), 'post', '', array('id' => 'upload_form2'));

if ($unsetusername) {
    unset($filecolumns['username']);
}
if ($unsetfirstname) {
    unset($filecolumns['firstname']);
}
if ($unsetlastname) {
    unset($filecolumns['lastname']);
}
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    $ignorelines = explode(',', $_POST['ignorelines']);

    $optype = $formdata->uutype;

    $uutype               = $formdata->uutype;
    ...
    $cohorts        = array();
    $rolecache      = uu_allowed_roles_cache_innoverz(); // Course roles lookup cache.
    $sysrolecache   = uu_allowed_sysroles_cache_innoverz(); // System roles lookup cache.
    ...
    $upt = new uu_progress_tracker_innoverz();
    ...
    while ($line = $cir->next()) {
        ...
        //added by Felix for ignoring users
        if (in_array($linenum, $ignorelines)) {
            continue;
        }
        ...
        //added by Felix
        if (!setAllValidDBFields($user, $optype, !in_array($linenum, $_SESSION['validlines']))) {
            continue;
        }
        ...
        if ($optype == UU_USER_ADDNEW or $optype == UU_USER_ADDINC) {
            ...
            if (!isset($user->firstname) or $user->firstname === '') {
                //innoverz: we don't need firstname and last name so this is not an error
                /*
        ...
        if ($existinguser) {
            ...
            if (($uutype == UU_USER_ADD_UPDATE or $uutype == UU_USER_UPDATE or $updatetype != UU_UPDATE_NOCHANGES) and !$remoteuser) {
            ...
            if (empty($user->email)) {
                ...
                $upt->track('status', $strusernotaddederror, 'error');
                //innoverz: already checked in preview page, no need to show error again in result page
                //$userserrors++;
                continue;
            ...
            //added by Felix
            $user->id = createUploadUser($user, false, false);
            $upt->track('username', html_writer::link(new moodle_url('/user/profile.php', array('id' => $user->id)), s($user->username)), 'normal', false);

            // add default value for required profile_fields for SWD users
            $user->profile_field_orgname = 'SWD';
            $user->profile_field_posttitle = '';
            $user->profile_field_officephone = '';
            $user->profile_field_officefax = '';
        ...
        //innoverz: role of uploaded users is always SWD General Learner (glswd)
        $filecolumns[] = 'sysrole1';
        $user->sysrole1 = 'glswd';
                ...
                if (!empty($user->$column)) {
                    ...
                    if ($removing) {
                        if (user_has_role_assignment($user->id, $sysroleid, SYSCONTEXTID)) {
                            ...
                            $upt->track('enrolments', get_string(
                                'unassignedsysrole',
                                'tool_uploaduser',
                                $sysrolecache[$sysroleid]->name
                            ));
                        }
                    } else {
                        if (!user_has_role_assignment($user->id, $sysroleid, SYSCONTEXTID)) {
...
// added by Felix
//if (!isset($data)) {
unset($_SESSION['ignorekeys']);
$cirdata = array(); //keep user record for checking whether exist muti user record in csv
$ignoredata = array(); //keep user records which would be ignored
$ignorekeys = array();
$ignorelines = array();
$dataindex = -1;
$data = array(); //store valid upload user to print
$databack = array(); //store valid upload user not to print
$cir->init();
$linenum = 1; //column header is first line
while ($linenum <= $insertrows and $fields = $cir->next()) {
    $dataindex++;
    $linenum++;
    $rowcols = array();
    ...
    //added by Felix
    //$cirdatakey = $rowcols['engname'] . $rowcols['joindate'];
    $cirdatakey = $rowcols['profile_field_engname'] . $rowcols['profile_field_joindate'];

    if (isset($rowcols['email'])) {
        $isignore = false;
        // check if email is valid
        if (!validate_email($rowcols['email'])) {
            $rowcols['status'] = '<span class="warning-block">' . get_string('invalidemail') . '</span>';
            $isignore = true;
        }

        // check if email is duplicated
        if ($DB->record_exists('user', array('email' => $rowcols['email']))) {
            $rowcols['status'] = '<span class="warning-block">' . $stremailduplicate . '</span>';
            $isignore = true;
        }
        if ($isignore) {
            $rowcols['rejectupload'] = 1;
            $ignoredata[] = $rowcols;
            $ignorekeys[$cirdatakey] = 1;
            continue;
        }
    }

    // check duplicate records exist in Excel
    if (isset($cirdata[$cirdatakey])) {
        //print_r($rowcols);
        if ((isset($data[$cirdata[$cirdatakey]]) || isset($databack[$cirdata[$cirdatakey]]))  && !isset($ignorekeys[$cirdatakey])) {
            $thisdata = isset($data[$cirdata[$cirdatakey]]) ? $data[$cirdata[$cirdatakey]] : $databack[$cirdata[$cirdatakey]];
            $thisdata['status'] = '<span class="warning-block">' . get_string('duplicateuserinfile', 'local_admintool') . '</span>';
            $ignoredata[] = $thisdata;
            $ignorelines[] = $thisdata['line'];
            unset($data[$cirdata[$cirdatakey]]);
            unset($databack[$cirdata[$cirdatakey]]);
        }
        $rowcols['status'] = '<span class="warning-block">' . get_string('duplicateuserinfile', 'local_admintool')  . '</span>';
        $ignoredata[] = $rowcols;
        $ignorekeys[$cirdatakey] = 1;
        $ignorelines[] = $linenum;
        continue;
    }
    if (count(getExistingUsers(filterUserData($rowcols)))) {
        $rowcols['status'] = '<span class="warning-block">' . get_string('duplicateuserindb', 'local_admintool') . '</span>';
        $ignoredata[] = $rowcols;
        $ignorekeys[$cirdatakey] = 1;
        continue;
    }

    $cirdata[$cirdatakey] = $dataindex;

    //$nametemp = explode(',',$rowcols['engname']);
    //$username = $nametemp[0];
    /* $rowcols['username'] = $rowcols['engname'];

    if (isset($rowcols['username'])) {
        //$stdusername = core_user::clean_field($rowcols['username'], 'username');
        $stdusername = $rowcols['username'];
    ...
    $noerror = uu_check_custom_profile_data_innoverz($rowcols) && $noerror;
    $rowcols['status'] = implode('<br />', $rowcols['status']);
    if ($linenum - 1 <= $previewrows) {
        $data[$dataindex] = $rowcols;
    } else {
        $databack[$dataindex] = $rowcols;
    }
}
if ($fields = $cir->next()) {
...
$totalrecordnum = $linenum - 1;
$fullcirdata = $data;
$i = 0;
//may do not executed never
while ($fields = $cir->next()) {
    if ($i == 0)
        $data[$dataindex++] = array_fill(0, count($fields) + 2, '...');

    $fullcirdata[$dataindex++] = setUserColumns($filecolumns, $fields);
    $totalrecordnum++;
    $i++;
}
$cir->close();
...
foreach ($filecolumns as &$column) {
    $column = str_replace('profile_field_', '', $column);
    $table->head[] = str_replace('profile_field_', '', $column);
}
...
$a = new stdclass;
$a->display = count($data);
$a->total = $totalrecordnum;
echo html_writer::tag('div', get_string('displaytotalrecords', 'local_admintool', $a), array('class' => 'tablebottominfo'));
echo html_writer::end_tag('div');

// Print the user list which will be ignored
if (count($ignoredata) > 0) {
    printIgnoredUserList($filecolumns, $ignoredata);
    $_SESSION['ignorekeys'] = $ignorekeys;
    $validlines = array();
    foreach ($data as $val) {
        $validlines[] = $val['line'];
    }
    $_SESSION['validlines'] = $validlines;
} else {
    $_SESSION['validlines'] = array();
}
