...
if ($returnto === 'url' && confirm_sesskey() && $returnurl) {
    ...
    if (!empty($id)) {
        ...
        $returnurl = new moodle_url($CFG->wwwroot . '/local/course/'); //innoverz
    }
    if ($returnto !== 0) {
        switch ($returnto) {
            case 'category':
...
$PAGE->set_url('/local/course/edit.php', $pageparams); //innoverz
...
$editform = new local_course_edit_form(null, $args);
