 ...
 *
 * @module moodle-local_course-categoryexpander
 */
 ...
 *
 * @constructor
 * @class Y.Moodle.local_course.categoryexpander
 */
var CSS = {
  CONTENTNODE: 'content',
  ...
  PARENTWITHCHILDREN: '.category'
    ...
    NS = Y.namespace('Moodle.local_course.categoryexpander'),
    TYPE_CATEGORY = 0,
    TYPE_COURSE = 1,
    URL = M.cfg.wwwroot + '/local/course/category.ajax.php';
/**
...
NS.init = function () {
  ...
  doc.delegate('click', this.collapse_expand_all, SELECTORS.COLLAPSEEXPAND, this); // Only set up they keybaord listeners when tab is first pressed - it
...
NS.setup_keyboard_listeners = function () {
  var doc = Y.one(Y.config.doc);
  Y.log('Setting the tabindex for all expandable course nodes', 'info', 'moodle-local_course-categoryexpander');
...
NS._toggle_coursebox_expansion = function (e) {
  var courseboxnode; // Grab the parent category container - this is where the new content will be added.
...
NS._toggle_category_expansion = function (e) {
  var categorynode, categoryid, depth;
...
NS._toggle_generic_expansion = function (config) {
  ...
  /**
   * innoverz start: add viewtype param
   */
...
NS.run_expansion = function (categorynode) {
  var categorychildren = categorynode.one(SELECTORS.CONTENTNODE),
      self = this,
      ancestor = categorynode.ancestor(SELECTORS.COURSECATEGORYTREE); // Add our animation to the categorychildren.

  this.add_animation(categorychildren); // If we already have the class, remove it before showing otherwise we perform the
  ...
  }, categorychildren.fx, categorychildren); // Now that everything has been set up, run the animation.
...
NS.expand_all = function (ancestor) {
  var finalexpansions = [];
  ancestor.all(SELECTORS.CATEGORYWITHCOLLAPSEDCHILDREN).each(function (c) {
  ...
  }, this); // Run the final expansion with animation on the visible items.
...
NS.collapse_all = function (ancestor) {
  var finalcollapses = [];
  ancestor.all(SELECTORS.CATEGORYWITHMAXIMISEDLOADEDCHILDREN).each(function (c) {
  ...
  }, this); // Run the final collapses now that the these are hidden hidden.
...
NS.update_collapsible_actions = function (ancestor) {
  ...
  if (foundmaximisedchildren) {
    ...
    togglelink.setHTML(M.util.get_string('collapseall', 'moodle')).addClass(CSS.COLLAPSEALL).removeClass(CSS.DISABLED);
  } else {
    // No maximised children found but there are collapsed children. Show the expandall.
    togglelink.setHTML(M.util.get_string('expandall', 'moodle')).removeClass(CSS.COLLAPSEALL).removeClass(CSS.DISABLED);
  }
};
/**
...
NS.process_results = function (tid, response, args) {
  ...
  newnode = Y.Node.create(data); // Append to the existing child location.

  args.childnode.appendChild(newnode); // Now that we have content, we can swap the classes on the toggled container.

  args.parentnode.addClass(CSS.LOADED).removeClass(CSS.NOTLOADED); // Toggle the open/close status of the node now that it's content has been loaded.

  this.run_expansion(args.parentnode); // Remove the spinner now that we've started to show the content.
