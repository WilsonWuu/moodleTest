...
$isprinttransript = optional_param('download', 0, PARAM_INT);
$ishdvideo = optional_param('ishdvideo', 0, PARAM_INT);
...
require_capability('local/videos:viewvideoresources', context_system::instance());
...
if ($isprinttransript) {
    if (!$file = get_file_by_course_module($cm->id, 'text/plain')) {
        $file = get_file_by_course_module($cm->id);
    }
} else {
    $file = get_file_by_course_module($cm->id);
    $hdvideofile = get_file_by_course_module($cm->id, 'video/mp4', 'hdvideo');
    //$hashdversion = $hdvideofile ? true : false;
    if ($ishdvideo && $hdvideofile) {
        $file = $hdvideofile;
    }
}

$resource->mainfile = $file->get_filename();
$displaytype = $isprinttransript ? RESOURCELIB_DISPLAY_DOWNLOAD : resource_get_final_display_type($resource);
...
$link = $OUTPUT->image_url(get_string('imgnonconformance', 'local_videos'), 'theme');;
$PAGE->requires->data_for_js('isnonconformance', $resource->isnonconformance ? true : false);
$PAGE->requires->data_for_js('nonconformanceimagelink', $link->__toString());
$PAGE->requires->data_for_js('nonconformancepageid', $CFG->nonconformancepageid);
$PAGE->requires->js(new moodle_url($CFG->VIDEOS_BASEURL.'view.js'));
switch ($displaytype) {
    case RESOURCELIB_DISPLAY_EMBED:
        if ($isprinttransript) {
            resources_resource_display_embed($resource, $cm, $course, $file);
        } else {
            resource_video_display_embed($resource, $cm, $course, $file, !empty($hdvideofile), $ishdvideo);
            
            //2.7 streaming way is not working
            //stream_video($resource, $cm, $course, $file, !empty($hdvideofile), $ishdvideo);
        }
        break;
