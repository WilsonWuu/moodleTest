...
class reportlibrarypagehits_renderer extends reportlearningresources_renderer {
	
	var $roles;
	
	public function __construct() {
		$this->roles = get_all_roles();
		$this->tableattrs['id'] = 'reportlibrarypagehits';
	...
	private function get_table_header_fields() {
		$html = '
			<th class="completion-identifyfield" rowspan="2">'.get_string('subject','local_elibrary').'</th>
			<th class="completion-identifyfield center" colspan="'.count($this->roles).'">'.get_string('numofpagehits','local_elibrary').'</th>
			</tr>
			<tr>
		';
	...
	private function get_all_fields_printing($date, $hitslist, &$rowcount) {
		...
		if ($thislist) {
			foreach ($thislist as $row) {
				...
				$html .= '<td class="completion-progresscell">'.$row['name'].'</td>';
				$nums = $this->getNumofEachField($row['roles']);
