 ...
 */
defined('EDITOR_UNLIMITED_FILES') or define('EDITOR_UNLIMITED_FILES', -1);
 ...
 */
abstract class moodleform_innoverz
{
    /** @var string name of the form */
    protected $_formname;       // form name

    /** @var MoodleQuickForm_innvoverz quickform object definition */
    protected $_form;
    ...
    function __construct($action = null, $customdata = null, $method = 'post', $target = '', $attributes = null, $editable = true)
    {
        ...
        $this->_form = new MoodleQuickForm_innvoverz($this->_formname, $method, $action, $target, $attributes);
        if (!$editable) {
            $this->_form->hardFreeze();
        }

        $this->definition();
        $this->_draft = $this->get_draft();
        if ($this->is_load_from_draft()) {
            $this->set_data($this->_draft->draftdata);
        }

        $this->_form->addElement('hidden', 'formurl', $_SERVER['REQUEST_URI']);
        $this->_form->setType('formurl', PARAM_RAW);
    ...
    function get_html($in_data = null)
    {
        if (!is_null($in_data)) {
    ...
    /**
     * Added by Tai
     * Validate the form, and return the error messages
     *
     * @param bool $validateonnosubmit optional, defaults to false.  The default behaviour
     *             is NOT to validate the form when a no submit button has been pressed.
     *             pass true here to override this behaviour
     *
     * @return error messages if form data is not valid
     */
    function get_validate_messages($validateonnosubmit = false)
    {
        static $validated = null; // one validation is enough
     ...
     * @return bool true if a cancel button has been pressed
     */
    function is_savingdraft()
    {
        $mform = &$this->_form;
        if ($mform->isSubmitted()) {
            foreach ($mform->_draftButtons as $draftbutton) {
                if (optional_param($draftbutton, 0, PARAM_RAW)) {
                    return true;
                }
            }
        }
    ...
    function clean_draft($forceclean = false, $formurl = '')
    {
        if (!$forceclean && (!isset($_GET['cleandraft']) || $_GET['cleandraft'] != 1)) {
            return false;
        }
        global $DB;
        $url = '';
        if (!$forceclean) {
            ...
            $url = str_replace(array('&cleandraft=1', '?cleandraft=1'), '', $url);
        } else {
            $url = $formurl;
        }
        $DB->execute("DELETE FROM mdl_draft WHERE url=?", array($url));
        return true;
    }
    ...
    function get_data()
    {
        ...
        if (!$this->is_cancelled() and !$this->is_savingdraft() and $this->is_submitted() and $this->is_validated()) {
            $data = $mform->exportValues();
            ...
            if (empty($data)) {
                ...
                $this->clean_draft(true, $data['formurl']);
                return (object)$data;
            }
        } else {
            return NULL;
    ...
    function save_draft($data)
    {
        ...
        $dataobject->url = $data->formurl;
        $dataobject->draftdata = $draftdata = json_encode($data);
        $dataobject->addtime = time();
        $id = $DB->insert_record('draft', $dataobject);
        header("Location: " . $dataobject->url);
        exit;
        //return true;
    }
    ...
    {
        global $DB;
        $draft = $DB->get_record_sql("SELECT draftdata, addtime FROM mdl_draft WHERE url=? ORDER BY id DESC LIMIT 0, 1", array($_SERVER['REQUEST_URI']));
        if (!$draft) {
            return null;
        }
        $draft->draftdata = json_decode($draft->draftdata);
        return $draft;
    }

    function is_load_from_draft()
    {
        return isset($this->_draft);
    }
    ...
    function save_stored_file(
        ...
        if ($element instanceof MoodleQuickForm_filepicker) {
            ...
            $file_record = array(
                'contextid' => $newcontextid, 'component' => $newcomponent, 'filearea' => $newfilearea, 'itemid' => $newitemid,
    ...
    function display()
    {
        ...
        if ($this->is_load_from_draft()) {
            ...
            $string_data->date = date('Y-m-d H:ia', $this->_draft->addtime);

            $query_string = array();
            if (!empty($_SERVER['QUERY_STRING'])) {
                parse_str($_SERVER['QUERY_STRING'], $query_string);
            }
            $query_string['cleandraft'] = '1';
            $query_string = http_build_query($query_string);

            $string_data->url = $_SERVER['SCRIPT_NAME'] . (!empty($query_string) ? ('?' . $query_string) : '');

            echo html_writer::tag('div', get_string('load_from_draft', '', $string_data), array('class' => 'warningbar', 'style' => 'background-color:#FFFFA3;padding:5px;'));
        }
        $this->_form->display();
    }
    ...
    function add_checkbox_controller($groupid, $text = null, $attributes = null, $originalValue = 0)
    {
        ...
        $PAGE->requires->yui_module(
            'moodle-form-checkboxcontroller',
            'M.form.checkboxcontroller',
            array(
                array(
                    'groupid' => $groupid,
     ...
     */
    function add_action_buttons($cancel = true, $submitlabel = null, $draft = false)
    {
        ...
        if ($draft) {
            $buttonarray[] = &$mform->createElement('draft');
        }
        if ($cancel) {
     ...
     *                                              same page.
     */
    public static function mock_submit(
...
/**
 * MoodleQuickForm_innvoverz implementation
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_innvoverz extends HTML_QuickForm_DHTMLRulesTableless
{
     ...
     */
    function __construct($formName, $method, $action, $target = '', $attributes = null)
    {
        global $CFG, $OUTPUT;
        ...
        $this->_reqHTML = '<img class="req" title="' . get_string('requiredelement', 'form') . '" alt="' . get_string('requiredelement', 'form') . '" src="' . $OUTPUT->image_url('req', 'theme') . '" />';
        $this->_advancedHTML = '<img class="adv" title="' . get_string('advancedelement', 'form') . '" alt="' . get_string('advancedelement', 'form') . '" src="' . $OUTPUT->image_url('adv', 'theme') . '" />';
        $this->setRequiredNote(get_string('somefieldsrequired', 'form', '<img alt="' . get_string('requiredelement', 'form') . '" src="' . $OUTPUT->image_url('req', 'theme') . '" />'));
    }
    ...
    function accept(&$renderer)
    {
        ...
        if (method_exists($renderer, 'setCollapsibleElements') && !$this->_disableShortforms) {
            ...
            foreach (array_keys($this->_elements) as $elementIndex) {
                $element = &$this->_elements[$elementIndex];
                if ($element->getType() == 'header') {
                    $element->setText('<span class="formheader">' . $element->_text . '</span>');
                }
            }
        }
     ...
     * @see MoodleQuickForm_innvoverz::setType
     */
    ...
    public function getCleanedValue($value, $type)
    {
        if (is_array($type) && is_array($value)) {
            ...
            throw new coding_exception('Unexpected type or value received in MoodleQuickForm_innvoverz::getCleanedValue()');
        }
        return $value;
    }
...
//]]>
    ...
    function _getElNamesRecursive($element)
    {
        ...
        if (is_a($element, 'HTML_QuickForm_group')) {
            ...
            method_exists($element, 'getPrivateName') &&
    ...
    /**
     * Registers a button as draft button
     *
     * @param string $addfieldsname name of the button
     */
    function _registerDraftButton($addfieldsname)
    {
        $this->_draftButtons[] = $addfieldsname;
    }
...
/**
 * MoodleQuickForm_innvoverz renderer
 *
 * A renderer for MoodleQuickForm_innvoverz that only uses XHTML and CSS and no
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_Renderer_innoverz extends HTML_QuickForm_Renderer_Tableless
{
    ...
    function __construct()
    {
        ...
        $this->_elementTemplates = array(
            ...
            'static' => "\n\t\t" . '<div class="fitem {advanced} {emptylabel}"><div class="fitemtitle"><div class="fstaticlabel"><span class="formlabel">{label}<!-- BEGIN required -->{req}<!-- END required -->{advancedimg} </span>{help}</div></div><div class="felement fstatic <!-- BEGIN error --> error<!-- END error -->"><!-- BEGIN error --><span class="error">{error}</span><br /><!-- END error -->{element}</div></div>',

            'warning' => "\n\t\t" . '<div class="fitem {advanced} {emptylabel}">{element}</div>',

            'nodisplay' => '',

            'group' => "\n\t\t" . '<div id="{id}" class="fitem {advanced}<!-- BEGIN required --> required<!-- END required --> fitem_{type} {emptylabel}"><div class="fitemtitle"><div class="fgrouplabel"><span>{label}<!-- BEGIN required -->{req}<!-- END required -->{advancedimg} </span>{help}</div></div><fieldset class="felement {type}<!-- BEGIN error --> error<!-- END error -->"><!-- BEGIN error --><span class="error">{error}</span><br /><!-- END error -->{element}</fieldset></div>',

            'hiddenlabel' => "\n\t\t" . '<div id="{id}" class="fitem {advanced}<!-- BEGIN required --> required<!-- END required --> fitem_{type} {emptylabel}" {aria-live}><div class="fitemtitle"><span class="formlabel">{label}<!-- BEGIN required -->{req}<!-- END required -->{advancedimg} </span>{help}</div><div class="felement {type}<!-- BEGIN error --> error<!-- END error -->"><!-- BEGIN error --><span class="error">{error}</span><br /><!-- END error -->{element}<label class="accesshide">{label}</label></div></div>'
        );
     ...
     *
     * @param MoodleQuickForm_innvoverz $form reference of the form
     */
    ...
    function startGroup(&$group, $required, $error)
    {
        ...
        //added by felix WCAG issue
        $groupnames = array('servicesetting');
        if (in_array($group->getName(), $groupnames)) {
            $html = $this->_elementTemplates['group'];
        }
        ...
        if (
            in_array($group->getName(), $this->_stopFieldsetElements)
    ...
    function renderElement(&$element, $required, $error)
    {
        ...
        if ($element->getType() == 'filemanager' || $element->getType() == 'date_selector') {
            //added by feilx WCAG issue
            $html = $this->_elementTemplates['group'];
        } elseif ($element->getType() == 'checkbox') {
            $html = $this->_elementTemplates['hiddenlabel'];
    ...
    function finishForm(&$form)
    {
        ...
        if (!$form->isFrozen()) {
            ...
            if (count($args[1]) > 0) {
                $PAGE->requires->js_init_call('M.form.initFormDependencies', $args, true, moodleform_innoverz::get_js_module());
            }
        }
    }
    /**
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_Rule_Required_innoverz extends HTML_QuickForm_Rule
{
    /**
 ...
 */
$GLOBALS['_HTML_QuickForm_default_renderer'] = new MoodleQuickForm_Renderer_innoverz();

/** Please keep this list in alphabetical order. */
MoodleQuickForm_innvoverz::registerElementType('advcheckbox', "$CFG->libdir/form/advcheckbox.php", 'MoodleQuickForm_advcheckbox');
MoodleQuickForm_innvoverz::registerElementType('button', "$CFG->libdir/form/button.php", 'MoodleQuickForm_button');
MoodleQuickForm_innvoverz::registerElementType('cancel', "$CFG->libdir/form/cancel.php", 'MoodleQuickForm_cancel');
MoodleQuickForm_innvoverz::registerElementType('draft', "$CFG->libdir/form/draft.php", 'MoodleQuickForm_draft');
MoodleQuickForm_innvoverz::registerElementType('searchableselector', "$CFG->libdir/form/searchableselector.php", 'MoodleQuickForm_searchableselector');
MoodleQuickForm_innvoverz::registerElementType('checkbox', "$CFG->libdir/form/checkbox.php", 'MoodleQuickForm_checkbox');
MoodleQuickForm_innvoverz::registerElementType('date_selector', "$CFG->libdir/form/dateselector.php", 'MoodleQuickForm_date_selector');
MoodleQuickForm_innvoverz::registerElementType('date_time_selector', "$CFG->libdir/form/datetimeselector.php", 'MoodleQuickForm_date_time_selector');
MoodleQuickForm_innvoverz::registerElementType('duration', "$CFG->libdir/form/duration.php", 'MoodleQuickForm_duration');
MoodleQuickForm_innvoverz::registerElementType('editor', "$CFG->libdir/form/editor.php", 'MoodleQuickForm_editor');
MoodleQuickForm_innvoverz::registerElementType('filemanager', "$CFG->libdir/form/filemanager.php", 'MoodleQuickForm_filemanager');
MoodleQuickForm_innvoverz::registerElementType('filepicker', "$CFG->libdir/form/filepicker.php", 'MoodleQuickForm_filepicker');
MoodleQuickForm_innvoverz::registerElementType('grading', "$CFG->libdir/form/grading.php", 'MoodleQuickForm_grading');
MoodleQuickForm_innvoverz::registerElementType('group', "$CFG->libdir/form/group.php", 'MoodleQuickForm_group');
MoodleQuickForm_innvoverz::registerElementType('header', "$CFG->libdir/form/header.php", 'MoodleQuickForm_header');
MoodleQuickForm_innvoverz::registerElementType('hidden', "$CFG->libdir/form/hidden.php", 'MoodleQuickForm_hidden');
MoodleQuickForm_innvoverz::registerElementType('htmleditor', "$CFG->libdir/form/htmleditor.php", 'MoodleQuickForm_htmleditor');
MoodleQuickForm_innvoverz::registerElementType('listing', "$CFG->libdir/form/listing.php", 'MoodleQuickForm_listing');
MoodleQuickForm_innvoverz::registerElementType('modgrade', "$CFG->libdir/form/modgrade.php", 'MoodleQuickForm_modgrade');
MoodleQuickForm_innvoverz::registerElementType('modvisible', "$CFG->libdir/form/modvisible.php", 'MoodleQuickForm_modvisible');
MoodleQuickForm_innvoverz::registerElementType('password', "$CFG->libdir/form/password.php", 'MoodleQuickForm_password');
MoodleQuickForm_innvoverz::registerElementType('passwordunmask', "$CFG->libdir/form/passwordunmask.php", 'MoodleQuickForm_passwordunmask');
MoodleQuickForm_innvoverz::registerElementType('questioncategory', "$CFG->libdir/form/questioncategory.php", 'MoodleQuickForm_questioncategory');
MoodleQuickForm_innvoverz::registerElementType('radio', "$CFG->libdir/form/radio.php", 'MoodleQuickForm_radio');
MoodleQuickForm_innvoverz::registerElementType('recaptcha', "$CFG->libdir/form/recaptcha.php", 'MoodleQuickForm_recaptcha');
MoodleQuickForm_innvoverz::registerElementType('recaptchacustom', "$CFG->libdir/form/recaptchacustom.php", 'MoodleQuickForm_recaptchacustom');
MoodleQuickForm_innvoverz::registerElementType('select', "$CFG->libdir/form/select.php", 'MoodleQuickForm_select');
MoodleQuickForm_innvoverz::registerElementType('selectgroups', "$CFG->libdir/form/selectgroups.php", 'MoodleQuickForm_selectgroups');
MoodleQuickForm_innvoverz::registerElementType('selectwithlink', "$CFG->libdir/form/selectwithlink.php", 'MoodleQuickForm_selectwithlink');
MoodleQuickForm_innvoverz::registerElementType('selectyesno', "$CFG->libdir/form/selectyesno.php", 'MoodleQuickForm_selectyesno');
MoodleQuickForm_innvoverz::registerElementType('static', "$CFG->libdir/form/static.php", 'MoodleQuickForm_static');
MoodleQuickForm_innvoverz::registerElementType('submit', "$CFG->libdir/form/submit.php", 'MoodleQuickForm_submit');
MoodleQuickForm_innvoverz::registerElementType('submitlink', "$CFG->libdir/form/submitlink.php", 'MoodleQuickForm_submitlink');
MoodleQuickForm_innvoverz::registerElementType('tags', "$CFG->libdir/form/tags.php", 'MoodleQuickForm_tags');
MoodleQuickForm_innvoverz::registerElementType('text', "$CFG->libdir/form/text.php", 'MoodleQuickForm_text');
MoodleQuickForm_innvoverz::registerElementType('textarea', "$CFG->libdir/form/textarea.php", 'MoodleQuickForm_textarea');
MoodleQuickForm_innvoverz::registerElementType('url', "$CFG->libdir/form/url.php", 'MoodleQuickForm_url');
MoodleQuickForm_innvoverz::registerElementType('warning', "$CFG->libdir/form/warning.php", 'MoodleQuickForm_warning');

MoodleQuickForm_innvoverz::registerRule('required', null, 'MoodleQuickForm_Rule_Required_innoverz', "$CFG->dirroot/innoverz/lib/formslib.php");
