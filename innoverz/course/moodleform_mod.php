...
require_once($CFG->dirroot . '/innoverz/lib/formslib.php');
 ...
 */
abstract class moodleform_mod_innoverz extends moodleform_innoverz
{
    /** Current data */
    protected $current;
    /**
    ...
    function __construct($current, $section, $cm, $course, $action = 'modedit.php')
    {
        global $CFG;
    ...
    protected function init_features()
    {
        ...
        $this->_features->idnumber          = plugin_supports('mod', $this->_modname, FEATURE_IDNUMBER, false);
    ...
    function definition_after_data()
    {
        ...
        if ($completion->is_enabled() && plugin_supports('mod', $this->_modname, FEATURE_COMPLETION, false)) {    //Added by Tai
    ...
    function validation($data, $files)
    {
        ...
        if (
            ...
            if (
                empty($data['completionview']) && empty($data['completionusegrade']) &&
    ...
    function standard_coursemodule_elements()
    {
        ...
        if ($completion->is_enabled() && plugin_supports('mod', $this->_modname, FEATURE_COMPLETION, false)) {    //Added by Tai
            ...
            if ($gotcompletionoptions) {
                ...
                $mform->setDefault('completion', COMPLETION_TRACKING_AUTOMATIC);    //Added by Tai
            }
            ...
            // User point, added by Tai
            $mform->addElement('text', 'completionuserpoint', get_string('completionuserpoint', 'completion'), 'maxlength="5" size="5"');
            $mform->setType('completionuserpoint', PARAM_INT);
            $mform->setDefault('completionuserpoint', 0);

            // Completion questionnaire, added by Tai
            $add = optional_param('add', '', PARAM_ALPHA);
            $update = optional_param('update', 0, PARAM_INT);

            if ($add != 'questionnaire' && (!isset($this->_cm->modname) || $this->_cm->modname != 'questionnaire')) {
                if (!empty($add)) {    //Add
                    $mform->addElement('checkbox', 'completionquestionnaire', get_string('completionquestionnaire', 'completion'), get_string('completionquestionnaire_enable', 'completion'));
                } else {    //Update
                    $qcmid = 0;
                    if (isset($this->_cm->completionquestionnaire) && !empty($this->_cm->completionquestionnaire)) {
                        if ($qcm = $DB->get_record_sql("SELECT cm.id FROM mdl_course_modules cm LEFT JOIN mdl_modules m ON cm.module=m.id WHERE m.name='questionnaire' AND cm.instance={$this->_cm->completionquestionnaire}")) {
                            $qcmid = $qcm->id;
                            unset($qcm);
                        }
                    }

                    $mform->addElement('hidden', 'qcmid', $qcmid);
                    $mform->setType('qcmid', PARAM_INT);

                    $completionquestionnaireenable = 0;
                    $checkbox_attrs = array('id' => 'id_completionquestionnaireenable_checkbox', 'type' => 'checkbox', 'value' => '1');
                    if ($this->_cm->completionquestionnaireenable == 1) {
                        $completionquestionnaireenable = 1;
                        $checkbox_attrs['checked'] = true;
                    }

                    $mform->addElement('hidden', 'completionquestionnaireenable', $completionquestionnaireenable, array('id' => 'id_completionquestionnaireenable'));
                    $mform->setType('completionquestionnaireenable', PARAM_BOOL);

                    $HTML_completionquestionnaireenable = html_writer::tag('input', null, $checkbox_attrs);
                    $HTML_completionquestionnaireenable .= html_writer::tag('label', get_string('completionquestionnaire_enable', 'completion'), array('for' => 'id_completionquestionnaireenable_checkbox'));
                    $HTML_completionquestionnaireenable = html_writer::tag('span', $HTML_completionquestionnaireenable);

                    $HTML_completionquestionnairelink = '';
                    if ($qcmid) {
                        $HTML_completionquestionnairelink .= html_writer::link(new moodle_url('/mod/questionnaire/view.php', array('id' => $qcmid)), get_string('completionquestionnaire_view', 'completion'), array('target' => '_blank'));
                    }
                    $HTML_completionquestionnairelink = html_writer::tag('div', $HTML_completionquestionnairelink, array('id' => 'completionquestionnairelink'));

                    $HTML_completionquestionnaire = $HTML_completionquestionnaireenable . $HTML_completionquestionnairelink;
                    $mform->addElement('static', 'completionquestionnaire', get_string('completionquestionnaire', 'completion'), $HTML_completionquestionnaire);
                }
            }
        }
    ...
    function add_intro_editor($required = false, $customlabel = null, $defaultshowdesc = false)
    {
        ...
        if ($this->_features->showdescription  && $this->courseformat->has_view_page()) {
            //added by felix to set default show description and hidden checkbox
            if ($defaultshowdesc) {
    ...
    function add_action_buttons($cancel = true, $submitlabel = null, $submit2label = null)
    {
        ...
        //$submit3label = get_string('savechangesandsetupquestionnaire');
        ...
        if ($submit2label !== false && $this->courseformat->has_view_page()) {
            $element = &$mform->createElement('submit', 'submitbutton2', $submit2label);
            $attribute['class'] = 'btn btn-primary';
            $element->updateAttributes($attribute);
            $buttonarray[] = $element;
        }

        if ($submitlabel !== false) {
            $element = &$mform->createElement('submit', 'submitbutton', $submitlabel);
            $attribute['class'] = 'btn btn-primary';
            $element->updateAttributes($attribute);
            $buttonarray[] = $element;
        }

        //$buttonarray[] = &$mform->createElement('submit', 'submitbutton_setupauestionnaire', $submit3label);

        if ($cancel) {
            $element = &$mform->createElement('cancel');
            $attribute['class'] = 'btn btn-primary';
            $element->updateAttributes($attribute);
            $buttonarray[] = $element;
        }
